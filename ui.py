...
class _SysMetrics:
    def __init__(self):
        ...
        self.gpu  = -1.0
        self.tmp  = -1.0
        self._slow_tick = 0   # GPU/temp are polled every Nth cycle (heavy subprocess)
    ...
    def _update(self):
        ...
        # GPU + temperature spawn subprocesses (nvidia-smi / PowerShell-WMI) that
        # are far too heavy to run every 1.5 s — poll them every 6th cycle (~9 s)
        # and reuse the cached reading in between.  CPU/MEM/NET stay live (psutil).
        self._slow_tick = (self._slow_tick + 1) % 6
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None, initial: dict | None = None, mode: str = "init"):
        ...
        stt_row, self._stt_btns = _toggle_row(
            [("whisper","🎙 Whisper"), ("vosk","🔊 Vosk"), ("elevenlabs","⚡ ElevenLabs")],
        ...
        self._stt_model_lbl = _lbl("Model:", 7, col=C.TEXT_MED,
                                   align=Qt.AlignmentFlag.AlignRight)
        stt_detail.addWidget(self._stt_model_lbl)
        ...
        # Initial visibility — Whisper shows a size combo, Vosk a path box,
        # ElevenLabs (cloud Scribe) needs neither (uses the ElevenLabs API key).
        self._whisper_combo.setVisible(self._sel_stt == "whisper")
        self._vosk_model_input.setVisible(self._sel_stt == "vosk")
        self._stt_model_lbl.setVisible(self._sel_stt in ("whisper", "vosk"))
        ...
        # OpenAI API key — real api.openai.com needs it; LM Studio ignores a blank.
        # Only visible when the provider is the OpenAI-compatible one.
        self._openai_key_widget = QWidget()
        self._openai_key_widget.setStyleSheet("background: transparent;")
        ok_row = QHBoxLayout(self._openai_key_widget)
        ok_row.setContentsMargins(0, 0, 0, 0)
        ok_row.setSpacing(5)
        ok_row.addWidget(_lbl("OpenAI:", 7, col=C.TEXT_MED,
                              align=Qt.AlignmentFlag.AlignRight))
        self._openai_key_input = _input("OpenAI API key  (leave blank for LM Studio)", pw=True)
        self._openai_key_input.setText(_init.get("openai_api_key", ""))
        ok_row.addWidget(self._openai_key_input)
        layout.addWidget(self._openai_key_widget)
        self._openai_key_widget.setVisible(self._sel_llm_provider == "openai")
        ...
        tts_row, self._tts_btns = _toggle_row(
            [("edgetts","🔈 EdgeTTS"), ("kokoro","🤖 Kokoro"),
             ("elevenlabs","⚡ ElevenLabs"), ("muxlisa","🇺🇿 Muxlisa")],
        ...
        el_row.setSpacing(5)
        el_row.addWidget(_lbl("ElevenLabs:", 7, col=C.TEXT_MED,
                               align=Qt.AlignmentFlag.AlignRight))
        ...
        # Muxlisa speaker (1 = male, 0 = female) — only visible for Muxlisa TTS
        self._mux_speaker_widget = QWidget()
        self._mux_speaker_widget.setStyleSheet("background: transparent;")
        ms_row = QHBoxLayout(self._mux_speaker_widget)
        ms_row.setContentsMargins(0, 0, 0, 0)
        ms_row.setSpacing(5)
        ms_row.addWidget(_lbl("Speaker:", 7, col=C.TEXT_MED,
                              align=Qt.AlignmentFlag.AlignRight))
        self._mux_speaker_combo = QComboBox()
        self._mux_speaker_combo.setFixedHeight(28)
        self._mux_speaker_combo.setStyleSheet(_COMBO_STYLE)
        for val, label in [("1", "Male  (Sardor)"), ("0", "Female  (Madina)")]:
            self._mux_speaker_combo.addItem(label, userData=val)
        _cur_spk = str(_init.get("tts_speaker", "1"))
        for i in range(self._mux_speaker_combo.count()):
            if self._mux_speaker_combo.itemData(i) == _cur_spk:
                self._mux_speaker_combo.setCurrentIndex(i)
                break
        ms_row.addWidget(self._mux_speaker_combo)
        layout.addWidget(self._mux_speaker_widget)

        # Muxlisa API key — only visible for Muxlisa TTS
        self._mux_key_widget = QWidget()
        self._mux_key_widget.setStyleSheet("background: transparent;")
        mk_row = QHBoxLayout(self._mux_key_widget)
        mk_row.setContentsMargins(0, 0, 0, 0)
        mk_row.setSpacing(5)
        mk_row.addWidget(_lbl("Muxlisa:", 7, col=C.TEXT_MED,
                              align=Qt.AlignmentFlag.AlignRight))
        self._mux_key_input = _input("Muxlisa API key", pw=True)
        self._mux_key_input.setText(_init.get("muxlisa_api_key", ""))
        mk_row.addWidget(self._mux_key_input)
        layout.addWidget(self._mux_key_widget)
    ...
    def _update_tts_ui(self, key: str) -> None:
        ...
        is_muxlisa = (key == "muxlisa")

        # Kokoro = dropdown, Muxlisa = speaker picker (no free-text voice),
        # EdgeTTS / ElevenLabs = text voice input.
        if hasattr(self, "_tts_voice_input"):
            self._tts_voice_input.setVisible(not is_kokoro and not is_muxlisa)
        if hasattr(self, "_kokoro_combo"):
            self._kokoro_combo.setVisible(is_kokoro)

        self._voice_lbl.setVisible(not is_muxlisa)
        if key == "elevenlabs":
            ...
            if hasattr(self, "_tts_voice_input"):
                self._tts_voice_input.setPlaceholderText(
                    "en-US-GuyNeural  /  uz-UZ-SardorNeural  /  tr-TR-AhmetNeural  …"
                )

        if hasattr(self, "_kokoro_speed_widget"):
            self._kokoro_speed_widget.setVisible(is_kokoro)
        if hasattr(self, "_mux_speaker_widget"):
            self._mux_speaker_widget.setVisible(is_muxlisa)
        self._update_key_fields()

    def _update_key_fields(self) -> None:
        """API-key field visibility depends on BOTH the STT and TTS selections
        (the ElevenLabs key is shared by ElevenLabs STT and TTS) and the LLM
        provider — so it is centralised here and called from every setter."""
        stt  = getattr(self, "_sel_stt", "")
        tts  = getattr(self, "_sel_tts", "")
        prov = getattr(self, "_sel_llm_provider", "")
        if hasattr(self, "_el_key_widget"):
            self._el_key_widget.setVisible(stt == "elevenlabs" or tts == "elevenlabs")
        if hasattr(self, "_mux_key_widget"):
            self._mux_key_widget.setVisible(tts == "muxlisa")
        if hasattr(self, "_openai_key_widget"):
            self._openai_key_widget.setVisible(prov == "openai")
    ...
    def _set_llm_provider(self, key: str):
        ...
        if hasattr(self, "_llm_hint_lbl"):
            if key == "openai":
                self._llm_hint_lbl.setText("lmstudio.ai / api.openai.com  ·  set API key for real OpenAI")
            else:
    ...
    def _set_stt(self, key: str):
        ...
        for k, btn in self._stt_btns.items():
            ...
            btn.setStyleSheet(f"""
                ...
                QPushButton:hover {{ color: {C.TEXT}; border: 1px solid {C.BORDER_B}; }}
            """)
        # Toggle model selector widgets (ElevenLabs STT = cloud Scribe, no model field)
        ...
        if hasattr(self, "_stt_model_lbl"):
            self._stt_model_lbl.setVisible(key in ("whisper", "vosk"))
        self._update_key_fields()
    ...
    def _submit(self):
        ...
        # STT model: combo for Whisper, path for Vosk, none for ElevenLabs (cloud)
        if self._sel_stt == "whisper":
            stt_model = self._whisper_combo.currentText()
        elif self._sel_stt == "vosk":
            stt_model = self._vosk_model_input.text().strip()
        else:
            stt_model = ""   # ElevenLabs — leave any existing stt_model untouched

        # Voice: Kokoro = dropdown, Muxlisa = speaker picker, others = text input
        tts_speaker = ""
        ...
        elif self._sel_tts == "muxlisa":
            tts_voice   = ""    # not used by Muxlisa — leave existing untouched
            tts_speed   = "1.0"
            tts_speaker = self._mux_speaker_combo.currentData() or "1"
        else:
        ...
        cfg = {
            ...
            "muxlisa_api_key":    self._mux_key_input.text().strip(),
            "openai_api_key":     self._openai_key_input.text().strip(),
        }
        if self._sel_stt == "vosk" and stt_model:
            cfg["vosk_model_path"] = stt_model

        # Drop blank values so the merge-save never clobbers an existing key
        # (API keys, stt_model, tts_voice, tts_speaker …) with an empty string.
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.on_ptt_start     = None   # push-to-talk pressed
        self.on_ptt_stop      = None   # push-to-talk released
        self.on_stop          = None   # Esc / interrupt
        self._muted           = False
        self._current_file: str | None = None
        ...
        sc_stop.activated.connect(self._fire_stop)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._ptt_btn.pressed.connect(self._ptt_pressed)
        self._ptt_btn.released.connect(self._ptt_released)
    ...
    def _style_ptt_btn(self, active: bool):
        if active:
            self._ptt_btn.setText("●  GAPIRING…  (qo'yvorsangiz bajaradi)")
            ...
            self._ptt_btn.setText("🎤  BOSIB TURIB GAPIR")
            self._ptt_btn.setStyleSheet(f"""
                QPushButton {{
                    background: {C.PANEL}; color: {C.PRI};
                    border: 1px solid {C.PRI_DIM}; border-radius: 3px;
    ...
    def _save_config_merged(self, cfg: dict) -> dict:
        """Merge *cfg* INTO the existing api_keys.json instead of overwriting it,
        so keys the overlay doesn't manage (openai_api_key, muxlisa_api_key,
        tts_speaker, vision_model, …) are preserved rather than silently wiped."""
    ...
    def _on_config_done(self, config_json: str):
        try:
            ...
            cfg = {}
        merged = self._save_config_merged(cfg)
        if self._overlay:
            self._overlay.hide()
            self._overlay = None
        llm = merged.get("llm_model", "")
        stt = merged.get("stt_engine", "")
        tts = merged.get("tts_engine", "")
...
class JarvisUI:
    ...
    def on_ptt_start(self):
        return self._win.on_ptt_start

    @on_ptt_start.setter
    def on_ptt_start(self, cb):
        self._win.on_ptt_start = cb

    @property
    def on_ptt_stop(self):
        return self._win.on_ptt_stop

    @on_ptt_stop.setter
    def on_ptt_stop(self, cb):
        self._win.on_ptt_stop = cb
