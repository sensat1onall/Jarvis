...
def _play_samples(samples, sample_rate: int) -> None:
    """Play float32 mono audio.

    On Windows the long-running PyQt process + persistent mic InputStream can
    leave PortAudio's global sd.play() stream silent.  We therefore play via the
    OS-native winsound backend (totally independent of PortAudio) on Windows,
    and fall back to sounddevice elsewhere or if winsound fails.
    """
    import sys
    ...
    if sys.platform == "win32":
        try:
            ...
            winsound.PlaySound(buf.getvalue(), winsound.SND_MEMORY)  # blocks until done
            return
        except Exception as e:
            print(f"[TTS] winsound failed, falling back to sounddevice: {e}")
    sd.play(arr, sample_rate)
    sd.wait()


def _play_audio_bytes(audio_bytes: bytes) -> None:
    """Decode MP3/WAV/OGG bytes (via miniaudio) and play."""
    import miniaudio
    ...
    _play_samples(samples, decoded.sample_rate)


def stop_playback() -> None:
    """Immediately halt any audio currently playing — for barge-in / 'Jarvis Stop'.
    winsound.PlaySound(None, 0) cancels an in-progress SND_MEMORY clip on Windows;
    sd.stop() covers the sounddevice fallback path on every OS."""
    try:
        import sys
        if sys.platform == "win32":
            import winsound
...
class MuxlisaTTSEngine:
    """Uzbek-native TTS via Muxlisa AI (service.muxlisa.uz). API key + internet required.

    speaker: 0 = female, 1 = male (default). API caps text at 512 chars/request, so long
    text is chunked at word boundaries. Response is .wav → played via _play_audio_bytes.
    """

    _URL = "https://service.muxlisa.uz/api/v2/tts"
    _MAX = 500  # API hard limit is 512 — leave a margin

    def __init__(self, api_key: str, speaker: int = 1):
        if not api_key:
            raise RuntimeError("Muxlisa TTS needs an API key — set 'muxlisa_api_key' in config.")
        self.api_key = api_key
        try:
    ...
    def speak(self, text: str) -> None:
        ...
        if not text:
            return
        for chunk in self._chunks(text, self._MAX):
            resp = requests.post(
                self._URL,
                headers={"x-api-key": self.api_key, "Content-Type": "application/json"},
                json={"text": chunk, "speaker": self.speaker},
                timeout=30,
            )
            resp.raise_for_status()
            if resp.content:
                _play_audio_bytes(resp.content)   # .wav → miniaudio decode → play
...
def create_tts_player(config: dict) -> TTSPlayer:
    ...
    elif engine_name == "muxlisa":
        api_key = config.get("muxlisa_api_key", "")
        speaker = config.get("tts_speaker", 1)   # 0=female, 1=male
