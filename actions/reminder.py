...
# Inlined (stdlib-only) into every fired-reminder script so it ALSO drops the
# message into the running app's speech inbox — making JARVIS SPEAK the reminder
# aloud, not just show a toast.  References the `message` variable defined just
# above it in the generated script.  Kept here (not f-string-interpolated) so its
# own braces survive.
_SPEAK_IPC = """try:
    import json as _j, time as _t, os as _o, pathlib as _p
    _inbox = _p.Path.home() / ".jarvis" / "speech_inbox"
    _inbox.mkdir(parents=True, exist_ok=True)
    _f = _inbox / ("spk_%s_%d.json" % (_t.strftime("%Y%m%d_%H%M%S"), _o.getpid()))
    _f.write_text(_j.dumps({"text": "Eslatma: " + message}, ensure_ascii=False), encoding="utf-8")
except Exception:
    pass
"""
...
def reminder(
    ...
    # Log to a central file so the morning briefing (and a future 'list my
    # reminders') can see today's upcoming reminders.  Best-effort, never fatal.
    try:
        log_path = Path.home() / ".jarvis" / "reminders.json"
        log_path.parent.mkdir(parents=True, exist_ok=True)
        arr = []
        if log_path.exists():
            arr = json.loads(log_path.read_text(encoding="utf-8"))
        ...
        log_path.write_text(
            json.dumps(arr[-50:], ensure_ascii=False, indent=2), encoding="utf-8"
        )
    except Exception:
        pass
