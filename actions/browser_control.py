...
# ---------------------------------------------------------------------------
# System-Chrome handoff — open a URL as a NEW TAB in the user's running Chrome
# ---------------------------------------------------------------------------
...
def open_in_system_chrome(url: str) -> str:
    """Open *url* in the user's normal Chrome.  Chrome's single-instance handoff
    turns this into a NEW TAB of the already-running window (it only starts a
    fresh Chrome when none is running).  Falls back to the OS default browser
    ...
    try:
        ...
        if exe:
            ...
            os.startfile(target)                      # default browser
        elif _OS == "Darwin":
...
def browser_control(
    ...
    # Plain navigation/search goes to the user's OWN Chrome (new tab in the
    # running window) instead of spawning an automation browser.  Only when a
    # Playwright session is already live do we keep it, so follow-up DOM actions
    # (click/type/get_text) still target the same automated page.
    if action in ("go_to", "search") and not _registry._sessions:
        if action == "go_to":
            result = open_in_system_chrome(params.get("url", ""))
        else:
            ...
            if not q:
                ...
                engine = (params.get("engine") or "google").lower().strip()
                base   = _SEARCH_ENGINES.get(engine, _SEARCH_ENGINES["google"])
                result = open_in_system_chrome(base + quote_plus(q))
        _log(player, result)
        return result
