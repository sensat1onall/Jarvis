...
# Window targeting
#
# Old behaviour used WScript.Shell.AppActivate(title) on Windows.  AppActivate
# falls back to "title ENDS WITH" matching, so AppActivate("Notepad") matched
# BOTH "Untitled - Notepad" and a user's "Todays To Doo's - Notepad" and
# silently activated the wrong one — a following type/clear_field then clobbered
# the user's document.  It also always returned a fake "Focused window: X" even
# when nothing matched.
#
# Fix: enumerate the real windows (pygetwindow), choose ONE deterministically
# (_pick_window), activate that exact window, and report the ACTUAL title — or a
# real failure.  _pick_window is pure and unit-tested offline (no live desktop).
# ---------------------------------------------------------------------------

# The assistant's own window (ui.py: "J.A.R.V.I.S — MARK XL") is never a target.
_SELF_WINDOW_MARKERS = ("j.a.r.v.i.s", "jarvis", "mark xl")
...
def _pick_window(windows: "list[dict]", query: str,
                 exclude_substrings: tuple = _SELF_WINDOW_MARKERS) -> "dict | None":
    """
    Deterministically choose the best window for *query*.  Pure; never raises.

    windows: [{"title": str, "active": bool, "visible": bool}, ...]
             (order = front-to-back Z-order as returned by the OS)
    Returns the chosen dict, or None if nothing suitable matched.

    Match precedence (case-insensitive): exact > starts-with > ends-with > contains.
    Within one tier the ACTIVE (foreground) window wins, then Z-order — so when
    several windows match, the one the user is actually looking at is chosen
    instead of an arbitrary background window.  Titleless, invisible, or self
    (assistant) windows are never chosen.
    """
    q = _norm(query)
    if not q:
        return None
    ...
    for i, w in enumerate(windows):
        t = _norm(w.get("title"))
        if not t or not w.get("visible", True):
            continue
        if any(ex in t for ex in exclude_substrings):
            continue
        if   t == q:          tier = 0
        elif t.startswith(q): tier = 1
        elif t.endswith(q):   tier = 2
        elif q in t:          tier = 3
        else:                 continue
        ranked.append((tier, 0 if w.get("active") else 1, i, w))
    if not ranked:
        return None
...
def _list_windows() -> "list[dict]":
    """Enumerate visible, titled windows via pygetwindow.  Returns [] when
    unavailable (Linux, or pygetwindow missing) so the caller falls back to an
    OS-native path.  Each dict carries the live window object under '_win'."""
    try:
    ...
    try:
        ...
        active_title = act.title if act else None
    except Exception:
        active_title = None
    try:
...
def _focus_window(title: str) -> str:
    ...
    # Preferred: deterministic pick over the real window list (Windows / macOS).
    wins = _list_windows()
    if wins:
        chosen = _pick_window(wins, title)
        if not chosen:
            return f"No open window matches '{title}'."
        full = chosen["title"]
        ok   = _activate_exact(full, chosen.get("_win"))
        time.sleep(0.2)
        return f"Focused window: {full}" if ok else f"Found '{full}' but could not focus it."

    # ---- OS-native fallbacks (pygetwindow unavailable, e.g. Linux) ----
    if os_name == "mac":
        script = (
    ...
    if os_name == "windows":
        # pygetwindow returned nothing (unusual) — last-resort AppActivate.
        try:
            esc = title.replace("'", "''")
            subprocess.run(
                ["powershell", "-NoProfile", "-NonInteractive", "-Command",
            ...
            time.sleep(0.3)
            return f"Focused window (fallback): {title}"
        except Exception as e:
...
def _screen_find(description: str) -> tuple[int, int] | None:
    try:
        ...
        text = call_vision(b64, prompt, mime="image/png", timeout=30)
...
def computer_control(
    ...
    try:
        # Safe targeting: if an input action is given an explicit window title,
        # focus that exact window FIRST (deterministic _focus_window) and abort
        # if it can't be focused — so type/click/clear never hit the wrong window.
        if action in ("type", "smart_type", "clear_field", "paste",
                      "click", "double_click", "right_click") and params.get("title"):
            _fr = _focus_window(str(params.get("title")).strip())
            if not _fr.startswith("Focused window:"):
                return f"Aborted '{action}': could not focus target window — {_fr}"
            time.sleep(0.25)
