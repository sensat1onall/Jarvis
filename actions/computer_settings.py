...
def take_screenshot():
    # Full-screen capture saved to a file — NOT Win+Shift+S, which opens the
    # region-snip overlay and gets stuck waiting for the user to drag a region.
    try:
        from actions.screenshot import screenshot as _shot
        _shot()
        return
    except Exception as e:
        print(f"[Settings] screenshot delegate failed: {e}")
    try:
        from pathlib import Path as _P
        from datetime import datetime as _dt
        out = _P.home() / "Pictures" / f"screenshot_{_dt.now():%Y%m%d_%H%M%S}.png"
        out.parent.mkdir(parents=True, exist_ok=True)
        pyautogui.screenshot(str(out))
    except Exception as e:
        print(f"[Settings] screenshot fallback failed: {e}")
